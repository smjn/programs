...
i::Parser Char Int
i xs
  |(null xs || ((not (xs=="0")) && (snd$head$nat)==0)) = []
  |otherwise = nat
  where nat = natural xs
...
displayCall (x,list) = if (null list) then x else App (x) (head list)

displayDef (func,body) = Fun (fst func) (snd func) (body)
...
expr =
  first(
    (sp (parenthesized expr <|> funcCall <|>ifpar<|> getVar <|> getBool <|> getList <|> getInt))
    <*>
    many(      
	(sp checkTok)
	<*>
